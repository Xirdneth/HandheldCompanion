...
namespace Watchdog.Lib.Server
{
    public class HeartbeatClient
    {
        ...
        public DateTime MonitorApplicationTime { get; set; }
    }
        ...
        private void OnClientMessage(NamedPipeConnection<string, string> connection, string message)
        {
            ...
            switch (command)
            {
                ...
                case (int)Commands.RequestKill:
                    ...
                    break;
                case (int)Commands.MonitorApplication:
                    {
                        var client = FindByName(connection.Name);
                        if (client == null) break;
                        if (args.Length < 2) return;
                        ...
                        client.MonitorApplication = bool.Parse(active);
                        client.MonitorApplicationTime = DateTime.Now;
                        Debug.WriteLine("received AddMonitoredApplication request");
                    }
                    break;
                default:
