...
namespace Watchdog.Lib.Server
{
    public class ApplicationHandler
    {
        ...
        public void Check()
        {
            ...
            var ProcessName = processes.Where(w => w.ProcessName.Contains(ApplicationName)).FirstOrDefault()?.ProcessName;

            var ClientFound = _heartbeatServer.FindByProcessName(ProcessName);

            if(ClientFound != null)
            {
                Active = ClientFound.MonitorApplication;            
            }
